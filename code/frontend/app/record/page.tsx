...
export default function RecordPage() {
  ...
  const saveRecording = async () => {
    ...
    try {
      ...
      const { conversationId, filename: uploadedFilename } = await uploadResponse.json();
      console.log('✅ Upload complete:', uploadedFilename);

      // Step 2: Transcribe the SAME audio blob (no re-fetch!)
      const transcribeFormData = new FormData();
      transcribeFormData.append('audio', audioBlob, uploadedFilename);
      transcribeFormData.append('conversationId', conversationId);

      const transcribeResponse = await fetch('/api/transcribe', {  // ← Changed endpoint
        method: 'POST',
